...
customer_rol_df.to_csv('/Users/razzak_lebbai/Desktop/customer_rol/data/customer_rol_processesd.csv', index=False)

##get only 2017 and 2016 data since other data are incomplete

customer_rol_df = customer_rol_df[(customer_rol_df.EXPIRATION_DATE_YEAR!=2015) &(customer_rol_df.EXPIRATION_DATE_YEAR!=2018)]
...
##for out out
agg_opt = customer_rol_df.groupby(['OPT_OUT_VALUE', 'EXPIRATION_DATE_YEAR', 'has_business'])[['has_business']].count().unstack()
...
agg_opt['Ret_Rate']= (agg_opt.loc[:,1].div(agg_opt.loc[:,[0,1]].sum(axis=1), axis=0)*100).round(1)
...
list_columns=['OPT_OUT_VALUE', 'ONLINE_SUB_CHANNEL', 'PROD_FMLY_GRP_MOD','PURCHASE_AR_FLAG']
list_df=[]
for col in list_columns:
    temp_df='agg_'+col
    temp_df = customer_rol_df.groupby([col, 'EXPIRATION_DATE_YEAR', 'has_business'])[['has_business']].count().unstack()
    ...
    temp_df['Ret_Rate']= (temp_df.loc[:,1].div(temp_df.loc[:,[0,1]].sum(axis=1), axis=0)*100).round(1)
    ##del temp_df['ratio_0']
...
for df in list_df:
    if df.index.names[0]=='ONLINE_SUB_CHANNEL':
        df = df.drop(['First Time Retail','Trialware', 'trialware opt-out'])
    elif df.index.names[0]=='PROD_FMLY_GRP_MOD':
        df = df.drop(['N1','NAT','NOF', 'NU','OLBU','NOBU','NSW'])
    else:
        df = df
        
     
    render_mpl_table(df.reset_index(), header_columns=0, font_size=14, col_width=3.5,path='/Users/razzak_lebbai/Desktop/customer_rol/src/analysis/plots/before_ret_yearly/table_'+df.index.names[0]+'.png')

  
     
    BarPlotWithGroupAnoNosci(df.loc[:,['Ret_Rate']], title='Retention rate based for '+df.index.names[0], \
                            path='/Users/razzak_lebbai/Desktop/customer_rol/src/analysis/plots/before_ret_yearly/retention_'+df.index.names[0]+'.png')
...
count=0
for sdf in list_df:
    render_mpl_table(sdf.reset_index(), header_columns=0, font_size=14, col_width=2.5, path='/Users/razzak_lebbai/Desktop/customer_rol/src/analysis/plots/before_ret_yearly/table_year_test_'+sdf.index.names[1]+'.png')
...
#    ##get retention 
        BarPlotWithGroupAnoNosci(df.loc[:,['Ret_Rate']], title='Retention rate based for '+df.index.names[1]+' Year: '+str(df.index[0][0]), colormap=cm.summer,\
                             path='/Users/razzak_lebbai/Desktop/customer_rol/src/analysis/plots/before_ret_yearly/retention_year_test_'+str(df.index[0][0])+'_'+df.index.names[1]+'.png')
#   
